...
from e_invoices.models import NumberDistribution, Company, CompanyNotificationConfig
...
def process_all_E0501_xml():
    ...
    for company in companies:
        ...
        if configs:
            input_dir_E0501 = configs.input_dir_E0501
            #xml_folder_path = r"C:\Users\waylin\mydjango\e_invoice\download\invoice_no"
            ns = {'e0501': 'urn:GEINV:E0501:4.0'}
            files = [f for f in os.listdir(input_dir_E0501) if f.lower().endswith('.xml')]
            
            for filename in files:
                full_path = os.path.join(input_dir_E0501, filename)
                try:
            ...
            print(f"⚠️ {company.company_name} 找不到通知設定，略過")

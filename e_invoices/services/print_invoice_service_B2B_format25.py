...
def draw_table_first_page(invoice, draw, start_cm, column_widths_cm, headers, data_rows, sales_amount, total_tax_amount, company_name, total_amount, company_identifier, company_address, wrap_text_fn=draw_wrapped_text):
    dpi = 300
...
def render_and_merge_invoices(invoice, output_folder, column_widths_cm, headers, all_data_rows,
    ...
    draw_table_first_page(invoice, draw, (1.3, 3.5), column_widths_cm, headers, first_page_rows,
                          sales_amount, total_tax_amount, company_name, total_amount,
    ...
    for pdf in page_files:
        merger.append(pdf)
    invoice_filename = f"{invoice.invoice_number}_{invoice.buyer.buyer_identifier}_{invoice.invoice_date.strftime('%Y%m%d')}.pdf"
...
def generate_invoice_B2B_format25_pdf(invoice, output_folder):
    column_widths = [7.2, 1.9, 1.9, 2.6, 4.7]
    headers = ["品名", "數量", "單價", "金額", "備註"]
    # all_items = [
    #     [f"商品名稱{i+1}\n編號{i+101}", random.randint(1, 9), f"{random.randint(10, 99)}.00", f"{random.randint(100, 999)}.00", "備註內容"]

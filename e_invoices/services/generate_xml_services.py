...
def generate_A0101_xml_files(invoice, output_dir, xsd_path):
    output_paths = []
    ...
    nsmap = {
        None: "urn:GEINV:eInvoiceMessage:A0101:4.0",
        "xsi": "http://www.w3.org/2001/XMLSchema-instance",
        
    }
    root = LET.Element("Invoice", nsmap=nsmap)
    root.set("{http://www.w3.org/2001/XMLSchema-instance}schemaLocation", "urn:GEINV:eInvoiceMessage:A0101:4.0 A0101.xsd")
    ...
    buyer = LET.SubElement(main, "Buyer")
    LET.SubElement(buyer, "Identifier").text = invoice.buyer.buyer_identifier.zfill(8)
    LET.SubElement(buyer, "Name").text = invoice.buyer.buyer_name
    ...
    # Write validated XML to file
    filename = f"A0101_{invoice.invoice_number}.xml"
    full_path = os.path.join(output_dir, filename)
    with open(full_path, "wb") as f:
        f.write(xml_bytes)
...
def generate_A0201_xml_files(invoice, output_dir, xsd_path):
    output_paths = []
    # Load XSD schema once
    with open(xsd_path, 'rb') as f:
        schema_doc = LET.parse(f)
    ...
    nsmap = {
        None: "urn:GEINV:eInvoiceMessage:A0201:4.0",
        "xsi": "http://www.w3.org/2001/XMLSchema-instance",
        
    }
    root = LET.Element("CancelInvoice", nsmap=nsmap)
    root.set("{http://www.w3.org/2001/XMLSchema-instance}schemaLocation", "urn:GEINV:eInvoiceMessage:A0201:4.0 A0201.xsd")

    
    LET.SubElement(root, "CancelInvoiceNumber").text = invoice.invoice_number
    LET.SubElement(root, "InvoiceDate").text = invoice.invoice_date.strftime("%Y%m%d")
    LET.SubElement(root, "BuyerId").text = invoice.buyer.buyer_identifier.zfill(8)
    LET.SubElement(root, "SellerId").text = invoice.company.company_identifier.zfill(8)
    LET.SubElement(root, "CancelDate").text = invoice.cancel_date.strftime("%Y%m%d")
    LET.SubElement(root, "CancelTime").text = invoice.cancel_time.strftime("%H:%M:%S")
    LET.SubElement(root, "CancelReason").text = invoice.cancel_reason
    ...
    # Write validated XML to file
    filename = f"A0201_{invoice.invoice_number}.xml"
    full_path = os.path.join(output_dir, filename)
    with open(full_path, "wb") as f:
        f.write(xml_bytes)
...
def generate_A0301_xml_files(invoice, output_dir, xsd_path):
    return

def generate_A0102_xml_files(invoice, output_dir, xsd_path):
    output_paths = []
    # Load XSD schema once
    with open(xsd_path, 'rb') as f:
        schema_doc = LET.parse(f)
    ...
    nsmap = {
        None: "urn:GEINV:eInvoiceMessage:A0102:4.0",
        "xsi": "http://www.w3.org/2001/XMLSchema-instance",
        
    }
    root = LET.Element("InvoiceConfirm", nsmap=nsmap)
    root.set("{http://www.w3.org/2001/XMLSchema-instance}schemaLocation", "urn:GEINV:eInvoiceMessage:A0102:4.0 A0201.xsd")

    
    LET.SubElement(root, "InvoiceNumber").text = invoice.invoice_number
    LET.SubElement(root, "InvoiceDate").text = invoice.invoice_date.strftime("%Y%m%d")
    LET.SubElement(root, "BuyerId").text = invoice.buyer.buyer_identifier.zfill(8)
    LET.SubElement(root, "SellerId").text = invoice.company.company_identifier.zfill(8)
    LET.SubElement(root, "ReceiveDate").text = invoice.cancel_date.strftime("%Y%m%d")
    LET.SubElement(root, "ReceiveTime").text = invoice.cancel_time.strftime("%H:%M:%S")
    if invoice.buyer_remark:
        LET.SubElement(root, "BuyerRemark").text = invoice.buyer_remark
    if invoice.remark:    
    ...
    if not schema.validate(doc):
        errors = "\n".join([str(e) for e in schema.error_log])
        raise ValueError(f"XSD validation failed for accept invoice {invoice.invoice_number}:\n{errors}")

    # Write validated XML to file
    filename = f"A0102_{invoice.invoice_number}.xml"
    full_path = os.path.join(output_dir, filename)
    with open(full_path, "wb") as f:
        f.write(xml_bytes)
...
def generate_A0202_xml_files(invoice, output_dir, xsd_path):
    output_paths = []
    # Load XSD schema once
    with open(xsd_path, 'rb') as f:
        schema_doc = LET.parse(f)
    ...
    nsmap = {
        None: "urn:GEINV:eInvoiceMessage:A0202:4.0",
        "xsi": "http://www.w3.org/2001/XMLSchema-instance",
        
    }
    root = LET.Element("CancelInvoiceConfirm", nsmap=nsmap)
    root.set("{http://www.w3.org/2001/XMLSchema-instance}schemaLocation", "urn:GEINV:eInvoiceMessage:A0102:4.0 A0202.xsd")
    ...
    if not schema.validate(doc):
        errors = "\n".join([str(e) for e in schema.error_log])
        raise ValueError(f"XSD validation failed for confirm cancel invoice {invoice.invoice_number}:\n{errors}")

    # Write validated XML to file
    filename = f"A0202_{invoice.invoice_number}.xml"
    full_path = os.path.join(output_dir, filename)
    with open(full_path, "wb") as f:
        f.write(xml_bytes)
...
def generate_A0302_xml_files(invoice, output_dir, xsd_path):
    output_paths = []
    # Load XSD schema once
    with open(xsd_path, 'rb') as f:
        schema_doc = LET.parse(f)
    ...
    nsmap = {
        None: "urn:GEINV:eInvoiceMessage:A0302:4.0",
        "xsi": "http://www.w3.org/2001/XMLSchema-instance",
        
    }
    root = LET.Element("RejectInvoiceConfirm", nsmap=nsmap)
    root.set("{http://www.w3.org/2001/XMLSchema-instance}schemaLocation", "urn:GEINV:eInvoiceMessage:A0102:4.0 A0302.xsd")

    
    LET.SubElement(root, "RejectInvoiceNumber").text = invoice.invoice_number
    LET.SubElement(root, "InvoiceDate").text = invoice.invoice_date.strftime("%Y%m%d")
    LET.SubElement(root, "BuyerId").text = invoice.buyer.buyer_identifier.zfill(8)
    LET.SubElement(root, "SellerId").text = invoice.company.company_identifier.zfill(8)
    LET.SubElement(root, "RejectDate").text = invoice.reject_date.strftime("%Y%m%d")
    LET.SubElement(root, "RejectTime").text = invoice.reject_time.strftime("%H:%M:%S")
    if invoice.remark:    
        LET.SubElement(root, "Remark").text = invoice.remark
    ...
    if not schema.validate(doc):
        errors = "\n".join([str(e) for e in schema.error_log])
        raise ValueError(f"XSD validation failed for reject confirm invoice {invoice.invoice_number}:\n{errors}")

    # Write validated XML to file
    filename = f"A0302_{invoice.invoice_number}.xml"
    full_path = os.path.join(output_dir, filename)
    with open(full_path, "wb") as f:
        f.write(xml_bytes)
...
def generate_B0101_xml_files(invoice, output_dir, xsd_path):
    return

def generate_B0102_xml_files(invoice, output_dir, xsd_path):
    return

def generate_B0201_xml_files(invoice, output_dir, xsd_path):
    return

def generate_B0202_xml_files(invoice, output_dir, xsd_path):
    return